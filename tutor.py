...
def build_drill_prompt(
    ...
    *,
    ...
    return (
        ...
        f"• The phrase must make SENSE — something a person would actually say, whose "
        f"natural {name} rendering really does use \"{skill}\". Never force the "
        f"construction into a sentence it doesn't fit (e.g. a future time word with a "
        f"completed/experiential past marker); pick a different sentence instead.\n"
...
def build_lesson_drill_plan_prompt(
    ...
    *,
    ...
    return (
        ...
        f"• Each sentence must be one a native speaker would ACTUALLY say. The "
        f"construction has to FIT the meaning — never force it in. Time references "
        f"must agree with the tense/aspect marking (never a future time word with a "
        f"completed or experiential past marker, or the reverse: \"I have already been "
        f"to Japan tomorrow\" is nonsense). If a sentence only works with the "
        f"construction removed, write a different sentence instead.\n"
