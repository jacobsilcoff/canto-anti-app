...
def build_lesson_drill_prompt(
    ...
    *,
    ...
    return (
        ...
        f"answer. For the `note`: when WRONG, name the specific error first (e.g. 'Wrong "
        f"particle: 喺 not 係 for location', 'Missing aspect marker 咗') then state the rule "
        f"in one sentence. When CORRECT, confirm what the learner actually used (e.g. 'Good "
        f"— 本 is the correct classifier for books'). "
