...
def build_lesson_drill_prompt(
    ...
    *,
    ...
    return (
        ...
        f"rendering — don't nitpick word choice that means the same thing). "
        f"IMPORTANT: if the answer is correct except for missing or wrong diacritics/accents "
        f"(e.g. 'francaise' instead of 'française', 'cafe' instead of 'café'), mark it "
        f"CORRECT (`correct: true`) — the learner clearly knows the word/form. In the `note`, "
        ...
        f"'Incorrect conjugation of verb mettre', 'Wrong particle', 'Missing aspect marker'), "
        f"(2) explain WHY it's wrong and state the rule in one sentence (e.g. 'mettre → je "
        f"mets (not met) — first-person singular adds -s'). The learner needs to understand "
        f"the grammatical reason, not just see the correction. When CORRECT, confirm what "
        f"the learner actually used (e.g. 'Good — 本 is the correct classifier for books'). "
        f"When CORRECT: `corrected` must be the EXACT string the learner wrote (copy it "
        f"verbatim) — do not rephrase or tidy it. If there is another equally natural form "
        f"(e.g. 我本書 vs 我嘅書), put it in `alternative` with a one-sentence `alt_note` "
        f"on when to prefer each. Omit `alternative` when there is no meaningful variant. "
        f"When WRONG: `corrected` must be the FULL corrected sentence — the complete, "
        f"natural {name} translation of the English phrase you posed. NEVER return just the "
        f"corrected word or fragment; the learner sees it side-by-side with what they wrote, "
        f"so it must be a complete sentence they can compare against. "
