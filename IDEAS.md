...
**Phase 5 — gamification.** Make `/learn` *fun* like Duolingo. **✓ DONE (see Shipped — combo meter + XP tally + daily-goal ring):** (2) in-lesson combo meter (🔥 ×N chip + escalating XP + wrong-resets) and (3) daily-goal XP ring + animated end-of-lesson XP count-up with perfect-lesson bonus (XP persisted to `points_ledger` once per lesson). **STILL TODO:** (1) **skill-tree path** — replace the flat unit list with a vertical node path; each skill node carries **crown levels** (practice-again to level up → reuses the planner with `focus:"review"`); the deferred "first-class up-front unit rows / visible roadmap" lands here. **Complexity: Medium, frontend + a per-skill crown-level table.**
