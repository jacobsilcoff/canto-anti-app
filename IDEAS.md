...
- **Connect Facebook + find friends on the app** — A "Find friends from Facebook" section in the Friends modal (`messages.html`): OAuth-connect your Facebook account (`/api/social/facebook/login` → `/callback`, token stored Fernet-encrypted in `social_accounts`, migration 032), then "Find friends" surfaces your Facebook friends who already use the app (Graph `/me/friends` → mapped to our users via `db.get_users_by_social_ids`, minus existing friends/pending) with one-click Add. **Platform reality:** Meta removed full friend-list access in 2014/2018 — `/me/friends` returns ONLY mutual app users who granted `user_friends`, which requires Meta App Review (works for app devs/testers until then). Instagram exposes no contacts API at all, so it's not wired. Requires env `FACEBOOK_APP_ID`/`FACEBOOK_APP_SECRET` + the redirect URI `{APP_URL}/api/social/facebook/callback` whitelisted in the Meta app.
