...
- **Semantic label merge suggestions + LLM review** — Merge detection combines a string pass (affix-aware Levenshtein) with a **semantic embedding pass** (cosine ≥ 0.84, cached under sentinel lang `__label__`). Each candidate group is then vetted by a cheap LLM call (`translation.review_label_merge`, flash-lite) that rejects false positives (e.g. "expressing quantity" vs "expressing emotions" — same prefix, different concepts) and suggests a clean merged name. UI shows the LLM's suggested name in an editable input + reason; chips have × remove buttons (groups >2) so you can trim before merging. Groups are connected components (union-find, capped 2–6), sorted by confidence. Best-effort + unmetered.
