...
- **Textbook mid-page unit boundaries — split a shared page between two units** — Unit boundaries were whole-page integers, so a unit that starts/ends partway down a page dragged its neighbour's text into generation (unwanted vocab/content). Each chapter now carries optional `start_anchor`/`end_anchor` (one verbatim line, stored in `chapters_json`, no migration): `textbook.split_page`/`unit_pages`/`format_unit_source` trim the first page before the start anchor and the last page from the end anchor onward (line-based, whitespace-tolerant, safe when an anchor no longer matches). `_norm_chapters` lets two adjacent units share exactly the boundary page when either side is anchored (unanchored overlaps still resolve forward). The review sheet's **“Unit boundaries” split editor** renders the first/last page line-by-line; tapping a line sets the break and greys/strikes the neighbouring unit's portion (a JS mirror of the Python trimmer so preview == generation). Anchors ride in the generate payload and persist back to the detected chapter (debounced) so the split is remembered.
