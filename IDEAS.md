...
- **Reader import from URL or PDF** — Create a reading from a pasted article URL or an uploaded PDF instead of generating one. New `extract.py` (deterministic, no-LLM): SSRF-guarded `httpx` fetch + trafilatura article extraction for URLs, pypdf text extraction for PDFs (clean error on scanned/image-only PDFs). Routes `POST /api/reader/generate-from-url` and `POST /api/reader/generate-from-pdf`. A **"This is already in my target language"** checkbox skips the AI rewrite entirely (text used verbatim — saves a call); when unchecked, `translation.adapt_article_to_reading` translates + adapts the source into the target language at the chosen CEFR level — returning sentence-aligned `{target, english}` so the original source meaning is **pre-stored** as each sentence's translation (the reader shows it without re-translating target→English). Source text capped at 12k chars. UI: the reader generate form has **3 explicit modes** (✨ AI Story / 🔗 From URL / 📄 From PDF) — only AI Story shows difficulty + length; URL/PDF show just the source input + the "already in my target language" checkbox (no length; user-controllable difficulty deferred → #46, imports default to B1 when translated). **Lazy windowed preload:** the per-sentence translate+TTS preload (`/preload`) now takes `start`/`count` so the reader only pre-processes a render-ahead window (current page + 1) on open and per page-turn instead of every sentence — a 60-sentence article costs ~10 sentence-translations on open, not 60; the rest stays on-demand (tap-to-translate / on-demand audio).
