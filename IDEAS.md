...
- **Settings page redesign: category tabs** — Replaced the vertical scroll of ~12 uniform sections with horizontal tab navigation (Account, Learning, Preferences, AI, Admin). Only one category visible at a time, selected tab persisted to localStorage. Sections grouped logically: Profile+Plan+Appearance under Account, Study+Tutor Profile+Mastery+Deck Maintenance under Learning, Flashcards+Reader+Chat+Notifications under Preferences, Gemini API under AI. Admin tab hidden for non-admins.
- **Semantic label merge suggestions** — Merge detection now combines a string pass (affix-aware Levenshtein) with a **semantic embedding pass** so synonyms that share no letters are caught (e.g. "drinks"/"beverages", "movies"/"films"). Label names are embedded via the existing `embeddings.embed` (cached under a sentinel lang `__label__`, shared across users), and pairs with cosine ≥ 0.84 are connected. Groups are connected components over both signals (union-find, capped 2–6 members), sorted by confidence then card count. Best-effort + unmetered — degrades to string-only if no Gemini key.
