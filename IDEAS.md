...
Today the "already in my target language" import is used verbatim (authentic, but a real news article can be B2/C1+ and overwhelm a beginner). Offer an optional toggle to run imports through a CEFR-adapting rewrite (condense/simplify to the chosen level) instead of the literal sentence-by-sentence translation — trading fidelity/AI-saving for a level-tuned read.
