...
- **Tutor composer — drop the iOS form-accessory bar (contenteditable)** — On iPhone, focusing the composer showed iOS's form-assistant bar (‹ › prev/next + Done) above the keyboard. That bar is native and can't be removed for a real `<textarea>`/`<input>` in Safari, so the composer was switched to a `contenteditable="plaintext-only"` div (iOS doesn't attach the bar to non-form-fields). Kept all prior behavior (auto-grow one-line→cap, send button enable/disable, Enter-to-send/Shift+Enter newline, keyboard-fit) plus plain-text paste and a CSS `:empty::before` placeholder. Isolated commit so it's easy to roll back if iOS throws a curveball.
