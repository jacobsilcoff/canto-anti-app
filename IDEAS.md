...
- **Foundations module (script + sound system) — DESIGN LOCKED (2026-06), not yet built.** The writing/sound system is a different kind of learning (perceptual/production skill, not meaning), finite & factual, shared per-language, and **gates** the vocab course for non-Latin scripts. **Key architectural decisions:**
  ...
  - **Integration:** Foundations units sit at the FRONT of the `/learn` path, gating vocab for non-Latin scripts. **MVP = Korean/Hangul**; audio spike first (a lone jamo may not TTS well — use a representative syllable like 가 for ㄱ, specified in the curated `audio` field). The CEFR vocab syllabus already excludes script/sound.
