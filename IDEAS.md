...
- **Learn page polish + Gemini 429 retry** — Three fixes to the lessons page. (1) **429 rate-limit resilience**: `translation._call` / `_call_with_image` used to retry only on 503 (model overloaded); a 429 (rate-limit / quota — a `ClientError`, not `ServerError`) propagated straight to the user. Since the shared free-tier key is hammered by all metered users, a burst of lesson/translate calls tripped 429 constantly. Both now catch `APIError` and retry the retryable statuses (`429/500/502/503/504`) with a longer backoff (`1/3/8s`), turning most transient limits into a success. (2) **"📚 Words" review overlay** (`learn.html`): was a flat, unromanized, ungrouped pile — unreadable on mobile for non-Latin scripts. Words are now **grouped by the lesson that introduced them** (in teaching order via `course_concepts.introduced_lesson_id` → `lesson_num`/`lesson_title`, already returned by `db.get_course_vocab`), each word gets **romanization ruby** (offline oracle via the existing `applyRuby`), bigger type, and a cleaner In-deck/Not-yet badge. (3) **Course-header buttons**: the wrapping jumble of bordered `.course-regen` pills (Practice / Words / mute / unlock / Restart course) is now a tidy row of `.hdr-btn` chips — icon+label that collapse to 38px icon-only circles below 560px; Restart is an icon-only ↻ with danger styling, admin "Unlock all" is icon-only.
