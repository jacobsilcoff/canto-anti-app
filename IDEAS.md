...
- **Textbook lesson source review — multi-PDF library, editable parse, one approved lesson (#50)** — “＋ Add lesson” now offers **Continue my course** or **From a textbook**. The textbook path selects an existing parsed PDF or uploads another, then requires the learner to approve the exact page range and complete extracted text before generation. The text itself is editable (trim junk/fix extraction), optional instructions steer the lesson, and detected sections remain editable reusable presets rather than hidden generation batches. “Create lesson” plans and authors exactly one source-grounded lesson immediately; no chapter-wide queue or second “Add lesson” press. Books retain per-page cleaned text and AI-proposed section ranges for future lessons.
