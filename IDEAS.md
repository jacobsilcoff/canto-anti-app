...
- **Even glasses flashcard plugin (MentraOS) + API-token auth** — Review your due flashcards hands-free on Even Realities glasses, earning XP and syncing streak/quests exactly like the web. New self-contained MentraOS app in [`even-glasses/`](even-glasses/) (`@mentra/sdk` `AppServer`, TypeScript): reads a per-user Site URL + API token from MentraOS app settings, pulls `GET /api/cards/due`, and drives a **Pass/Fail** review loop on the monochrome HUD — tap to reveal, short-press = "got it" (→ SRS `good`, full XP), long-press = "missed it" (→ `again`) — POSTing each grade to `POST /api/cards/{id}/review` (which already awards XP + records study activity + bumps quests). Since the glasses have no speaker, **audio is never used**: non-Latin languages show **romanization** (as the pronunciation-face prompt, and in the reveal on other faces), and the audio-only **pronunciation face is skipped for Latin-alphabet languages** (via `/api/languages` `logographic`). To let the plugin call the API as you, the FastAPI app gained **long-lived bearer tokens**: `api_tokens` table (migration 037, stores `sha256(token)` like `sessions`), `db.create_api_token`/`get_user_by_api_token`/`revoke_api_tokens`, `auth_middleware` now accepts `Authorization: Bearer <token>`, and `POST`/`DELETE /api/profile/api-token` mint/revoke it (raw token shown once). Generated from **Settings → Even glasses**. Pure display logic (`even-glasses/src/cards.ts`) is unit-tested; the app-side token flow has `tests/test_api_token.py`.
