...
- **Textbook PDF → interactive lessons (📕 From textbook)** — Upload a grammar-book/textbook PDF on the Learn page and its chapters become queued interactive lessons. New `textbook.py`: `extract.extract_pdf` (now takes a `max_chars`; textbook cap 60k vs the reader's 12k) → one segmentation LLM call per ~9k-char chunk (`segment_textbook`, strictly normalized, book chapters merged across chunk boundaries, ≤40 lessons/import) → specs stored in the new `lesson_queue` table (migration 037). While the queue is non-empty, `main._author_next_lesson` consumes it FIFO **instead of calling the planner** (the book is the plan) and grounds the author prompt in the item's condensed `source` notes (the book's own rules/examples/vocab), so lessons match what the learner reads in the book. Each book chapter opens/closes as its own course unit (`unit_size` = chapter budget). Routes: `POST/DELETE /api/courses/{id}/textbook` (metered 1 unit per segmentation chunk; 402 pre-check); learn.html gets a "📕 From textbook" button + queued-lessons note with ✕ Clear. Authoring stays one-lesson-per-request via the existing "+ Add lesson"/auto-buffer flow, so a 30-lesson book never times out a request. Verified end-to-end (real routes + PDF, stubbed LLM): import → 2 queued → generation consumed the queue with 0 planner calls → chapters opened/closed per book unit.
