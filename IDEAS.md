...
- **Textbook lesson source review — multi-PDF library, editable parse, visuals, one approved lesson (#50)** — “＋ Add lesson” now offers **Continue my course** or **From a textbook**. Upload asks for an editable textbook name (also renameable later), then stores per-page cleaned text, AI-proposed section ranges, and useful page-linked embedded images/diagrams. The learner approves the exact pages, editable extracted text, up to six visual thumbnails, and optional instructions before generation. Selected visuals reach the planner as a labelled contact sheet. “Create lesson” plans/authors exactly one grounded lesson immediately; no chapter-wide queue or second button press. Vector-only PDF drawings are the known extraction limitation.
