...
- **Foundations module (separate from the main course)** — the writing/sound *system* itself (script + tones/pronunciation) is a different kind of learning: a perceptual/production skill, not meaning. So it lives in its own module keyed on `script_family` (+ tones), with its own concept kind (`grapheme`/`tone`/`phoneme`) and exercise types (character→sound recognition, sound→character, tone discrimination, tracing). It is **curated/data-driven, NOT AI-generated** — the inventory is finite & factual (the Hangul letters, the 6 Cantonese tones, the Devanagari/Telugu aksharas), it's the exact content AI got wrong in the spike (二→"yee5"), and our romanizers (`pycantonese`/`indic-transliteration`/…) already encode the grapheme→sound mappings so most of it can be derived. It is **shared per-language** (not per-user) and **gates** the main course for non-Latin scripts (learn Hangul before Korean vocab); for Latin scripts it's a short optional pronunciation primer. The main CEFR syllabus deliberately **excludes** script/sound and assumes the learner can read.
...
**Adding an exercise type:** (a) add its schema to `learning._EXERCISE_CONTRACT` + a romanisation plan in `_attach_romanization`; (b) add a renderer/grader entry to `EXERCISE_TYPES` in `static/learn.html` (contract: `render(ex,root,lang)->{isReady,grade,answerText,lock}`). Nothing else changes. **Foundations module** (script + tones/pronunciation; curated, gates non-Latin) is its own track — sequence it relative to the main build once we pick a starting language (Latin-script first sidesteps the gate; a non-Latin pilot needs Foundations first).
