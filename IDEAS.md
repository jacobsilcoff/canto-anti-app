...
- **Streak timezone bug fix** — `db.get_streak` computed "today" with Python `date.today()` (server *local* time) while activity is recorded with SQLite `date('now')` (*UTC*). On any non-UTC server the read/write day boundaries disagreed, so the consecutive-day count near midnight would miscount or reset to 0 — users lost streaks despite studying daily. `get_streak` now anchors to `datetime.now(timezone.utc).date()`, matching the writes (and the rest of the codebase, which uses `date('now')` for DAU / points_today / first_seen_date). Added `tests/test_streak.py` (6 cases) and `scripts/restore_streak.py` (idempotent study_activity backfill) to repair already-lost streaks.
