...
- **Profile pictures + first-name greeting** — Users can now set a profile picture (Settings → Account → Profile) shown in chat. Upload flow: `POST /api/profile/avatar` (multipart) center-crops to a square, resizes to 400px, JPEG-encodes, stores in the shared `media` table (`conv_id=NULL`) and points `users.avatar_media_id` (migration 036) at it; the old file is deleted on replace/remove (`DELETE /api/profile/avatar`). The picture is rendered as a `<img>` inside the existing circular `.msg-conv-avatar`/`.msg-chat-avatar` in `messages.html` (falls back to initials when unset; served via the existing unauth `/api/media/{id}.jpg`, exposed to chat peers through `db.list_conversations`' new `avatar_url`). Settings gets a live-preview circle + Upload/Remove. Separately, the Home greeting now uses the user's **first name** (split from `display_name`) — this also fixed a latent bug where `/api/me` never returned `display_name`, so the greeting always fell back to username.
