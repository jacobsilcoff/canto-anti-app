...
- **Unknown-word glossing (`vocab_glossary`)** — Every native token in teach blocks + word-bank tiles gets an English gloss if the learner doesn't already know it. Known-words set built deterministically by `db.get_known_words` from SRS cards + previously-taught course concepts; `assemble_lesson` filters the LLM-supplied glossary against it. Frontend `applyRuby` renders a per-token gloss label under unknown words (romanization-token cache decoupled from glossing).
