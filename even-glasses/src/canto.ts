/**
 * Thin client for the canto-anti-app REST API, authenticated with a long-lived
 * bearer token (generated in the site's Settings → Even glasses).
 *
...
export class CantoError extends Error {
  constructor(message: string, readonly status?: number) {
    super(message);
    this.name = "CantoError";
  }
}

export class CantoClient {
  ...
  private async req<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    try {
      ...
      throw new CantoError(`Can't reach the site (${(e as Error).message}).`);
    }
    if (res.status === 401) {
      throw new CantoError("Token rejected — regenerate it in Settings.", 401);
    }
    if (!res.ok) {
      throw new CantoError(`Request failed (${res.status}).`, res.status);
    }
