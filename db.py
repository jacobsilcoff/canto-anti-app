...
# ── API tokens ────────────────────────────────────────────────────────────────
# Long-lived bearer tokens for programmatic access (the Even glasses / MentraOS
# plugin). Like sessions, only sha256(token) is stored. These never expire — they
# are revoked explicitly. Generating a new one replaces any existing token(s) for
