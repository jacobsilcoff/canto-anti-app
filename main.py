...
async def auth_middleware(request: Request, call_next):
    ...
    # Programmatic clients (the Even glasses / MentraOS plugin) authenticate with
    # a long-lived bearer token instead of the browser session cookie.
    if user_id is None:
...
async def create_api_token(request: Request, user: dict = Depends(current_user)):
    """Mint a long-lived bearer token for the Even glasses / MentraOS plugin.
