...
def _build_lesson_prompt(
    ...
    return (
        ...
        f"DISTRACTORS — never an index; we build and shuffle the options.\n"
        f"ANSWER-KEY DISCIPLINE — there must be EXACTLY ONE correct option (an ambiguous "
        f"key mis-grades the learner):\n"
        f"• Every distractor must be unambiguously WRONG for this exact prompt — wrong "
        f"grammar, or wrong meaning here. NEVER use a distractor that is also an "
        f"acceptable answer.\n"
        f"• CLOZE: the surrounding sentence must FORCE one filler. Do not cloze a slot "
        f"where several taught words fit (e.g. greetings like Bonjour/Bonsoir/Salut are "
        f"interchangeable — \"___ Madame, comment allez-vous?\" accepts several). Either "
        f"add context that forces one (time of day, register, who's speaking) or test "
        f"that word with recognition/production/listening instead.\n"
        ...
        f"• {native_rule}\n"
        f"• Every drill tests a listed concept; EXACTLY ONE option is correct and every "
        f"distractor is unambiguously wrong here (never a second valid answer).\n\n"
        f"Return ONLY valid JSON, no other text:\n"
        '{\n'
