...
- **Structured side-channel, strictly normalized** (`tutor._normalize` — same trust model as lessons): `{reply, corrections≤3, new_items≤5, points≤3}`. Filter-then-clip (malformed entries don't consume slots), point values clamped 1–3, `romanization` on every new_item **recomputed by `tokenizer.romanize_text`** (never the model). JSON-parse failure retries once, then degrades to a plain-text reply — chat never hard-fails on the side-channel.
- **UI:** conversation chip strip (new/delete; first message becomes the title), per-bubble TTS via `/api/tts`, inline-ruby toggle (`tutor_ruby_inline` localStorage), correction cards, new-item chips with one-click **Add to deck** (`POST /api/cards`, deduped via `/api/cards/status`), point toasts bumping the ⭐ header total. 402/429 render as inline system bubbles. The ruby/tooltip helpers are copied from learn.html (a shared-file refactor is a known follow-up).
