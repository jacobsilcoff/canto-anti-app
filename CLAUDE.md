...
- **UI (ChatGPT-style mobile chat):** current-chat button opens a left **conversations drawer** (new/delete, ellipsis-truncated; first message becomes the title). the **thread is the only scroller**, the composer stays pinned. The **body is `position: fixed`** so the browser physically can't scroll the page — this is what stops iOS from scrolling up to "reveal the caret" on every keystroke (the old `scrollTo(0,0)` scroll-handler fought that and made the composer flash above the keyboard each keypress). A `visualViewport` handler (resize + focus only — no scroll handler) keeps the body **height** synced to the *visible* viewport so the layout (a flex column) **compresses above the mobile keyboard** instead of the composer sliding under it (iOS doesn't resize the layout viewport). **The tutor body must also override the global `body { min-height: 100dvh }` with `min-height: 0`** — `dvh` does NOT shrink for the keyboard, so the floor would otherwise pin the body at full height and the JS-set height could never compress the chat (symptom: the page scrolls and the composer ends up under the keyboard). A `body.kb-open` class (toggled when the visible viewport is >100 px shorter than the layout viewport) zeroes the composer's `env(safe-area-inset-bottom)` padding so it sits snug on the keyboard instead of floating a home-indicator gap above it. The composer textarea auto-grows from one line (44 px) to a cap relative to `visualViewport.height` (~22%, so it shrinks when the keyboard is up) — note it must set `min-height: 0` to override the global `textarea { min-height: 120px }`, else `scrollHeight` is floored at 120 px and the box can never shrink back to one line. Each tutor bubble has an icon-only tools row: **🔊 listen**, **Aa** whole-sentence English, **あ** per-bubble ruby toggle (reader-style, strike-through when off; default persisted in `tutor_ruby_inline`), and **🎯 Drill** (only when `drill` is set). **Corrections render right-aligned UNDER the learner's own message** (a `.corr-wrap` sibling inserted after the user bubble — a correction is about what they said, not the tutor's reply), collapsible. New-word suggestions are a collapsible section in the tutor bubble; fresh messages start open, history loads collapsed. New-item chips have one-click **Add to deck** (`POST /api/cards`, deduped via `/api/cards/status`) AND a **✕ dismiss**. Point toasts bump the ⭐ header total; 402/429 render as inline system bubbles.
