...
- **UI (ChatGPT-style mobile chat):** current-chat button opens a left **conversations drawer** (new/delete, ellipsis-truncated; first message becomes the title). the **thread is the only scroller**, the composer stays pinned. The mobile-keyboard handling is the **MDN `visualViewport` pattern** and is fiddly — three interlocking pieces, all required:
  ...
  A `body.kb-open` class (toggled when the visible viewport is >100 px shorter than the layout viewport) zeroes the composer's `env(safe-area-inset-bottom)` padding so it sits snug on the keyboard instead of floating a home-indicator gap above it. The composer is a **`contenteditable="plaintext-only"` div, NOT a `<textarea>`** — deliberately, so iOS doesn't attach its form-accessory bar (the ‹ › / Done strip above the keyboard, which only appears for real form fields). It auto-sizes to its content from one line (~47 px) and is capped by an inline `max-height` set relative to `visualViewport.height` (~22%, so it shrinks when the keyboard is up). Plumbing vs a textarea: read text with `input.innerText`, clear with `input.innerHTML=''`, placeholder via CSS `:empty::before { content: attr(data-placeholder) }`, a `paste` handler forces plain text, and an `input` handler strips a stray `<br>` (clears `innerHTML` when `textContent===''`) so the placeholder reappears. Each tutor bubble has an icon-only tools row: **🔊 listen**, **Aa** whole-sentence English, **あ** per-bubble ruby toggle (reader-style, strike-through when off; default persisted in `tutor_ruby_inline`), and **🎯 Drill** (only when `drill` is set). **Corrections render right-aligned UNDER the learner's own message** (a `.corr-wrap` sibling inserted after the user bubble — a correction is about what they said, not the tutor's reply), collapsible. New-word suggestions are a collapsible section in the tutor bubble; fresh messages start open, history loads collapsed. New-item chips have one-click **Add to deck** (`POST /api/cards`, deduped via `/api/cards/status`) AND a **✕ dismiss**. Point toasts bump the ⭐ header total; 402/429 render as inline system bubbles.
