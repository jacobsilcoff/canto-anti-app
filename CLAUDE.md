...
- **UI (ChatGPT-style mobile chat):** current-chat button opens a left **conversations drawer** (new/delete, ellipsis-truncated; first message becomes the title). `html/body` are locked to the viewport — the **thread is the only scroller**, the composer stays pinned. A `visualViewport` handler (resize + scroll + focus) pins the page to the *visible* viewport height/offset so the layout **compresses above the mobile keyboard** instead of the composer sliding under it (iOS doesn't resize the layout viewport); the textarea grow-cap is also relative to `visualViewport.height` (~22%) so it shrinks when the keyboard is up. Each tutor bubble has an icon-only tools row: **🔊 listen**, **Aa** whole-sentence English, **あ** per-bubble ruby toggle (reader-style, strike-through when off; default persisted in `tutor_ruby_inline`), and **🎯 Drill** (only when `drill` is set). **Corrections render right-aligned UNDER the learner's own message** (a `.corr-wrap` sibling inserted after the user bubble — a correction is about what they said, not the tutor's reply), collapsible. New-word suggestions are a collapsible section in the tutor bubble; fresh messages start open, history loads collapsed. New-item chips have one-click **Add to deck** (`POST /api/cards`, deduped via `/api/cards/status`) AND a **✕ dismiss**. Point toasts bump the ⭐ header total; 402/429 render as inline system bubbles.
