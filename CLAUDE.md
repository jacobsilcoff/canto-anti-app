...
- **Target-language-only reply + on-demand translation:** the `reply` is written **entirely** in the target language (English lives only in the structured fields). The same call returns `reply_en` (full sentence translation) and `gloss` ({native_word: English}, word-for-word) so the client offers reader-style reveal at no extra cost: jyutping ruby + per-word English on tap (`.gl` tooltip) + a per-bubble **"Aa"** toggle for the whole-sentence English. The prompt is tuned to be an engaging partner — open-ended questions (almost never yes/no), personality, cultural tidbits, pushing the learner to say more.
- **Structured side-channel, strictly normalized** (`tutor._normalize` — same trust model as lessons): `{reply, reply_en, gloss≤40, corrections≤3, new_items≤5, points≤3}`. Filter-then-clip (malformed entries don't consume slots), point values clamped 1–3, `romanization` on every new_item **recomputed by `tokenizer.romanize_text`** (never the model). **`new_items` are dropped if the learner already used the word in their message or has it in their deck** (`_strip_for_match` against `user_msg` + known words) — they earn points for those instead. JSON-parse failure retries once, then degrades to a plain-text reply — chat never hard-fails on the side-channel.
- **UI:** conversation chip strip (new/delete, ellipsis-truncated; first message becomes the title), per-bubble TTS via `/api/tts` + an **optional auto-play toggle** (`tutor_auto_tts`, default off — reads new replies aloud), inline-ruby toggle (`tutor_ruby_inline`), correction cards, new-item chips with one-click **Add to deck** (`POST /api/cards`, deduped via `/api/cards/status`), point toasts bumping the ⭐ header total. 402/429 render as inline system bubbles. **Mobile layout** is a bounded flex column (`min-height:0` chain) so the thread is the only scroller and the composer stays pinned. The ruby/tooltip/gloss helpers are copied from learn.html (a shared-file refactor is a known follow-up).
